...
$TimeSheets = DB::table('Timesheet_Item')->where(function($query){
...
})->where('Contact_ID',$Contact->Contact_ID)->join("Timesheet","Timesheet.Timesheet_ID","Timesheet_Item.Timesheet_ID")->join('Timesheet_Period','Timesheet_Period.Timesheet_Period_ID','Timesheet.Timesheet_Period_ID')->orderby('Timesheet_Period','desc')->get();
...
<?php 
	...
	$TotalTime = db::table('Timesheet_Item_Time')->whereIn('Timesheet_Item_ID',$Time)->select(DB::raw('SUM(Time_Basic) as Total'))->first();
      ...
      <td><a  href="LPT2/User/export?time={{$TimeSheet->Timesheet_ID}}&C_ID={{$Contact->Contact_ID}}&Name={{$Name}}&Company={{$Org->Name}}&Role={{$Role}}" class="green-text" data-id="{{$TimeSheet->Timesheet_ID}}"><i class="fas fa-file-excel"></i> View </a></td>
    </tr>
