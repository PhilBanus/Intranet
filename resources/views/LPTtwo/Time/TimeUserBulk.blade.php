...
$TimeItems = db::table('Timesheet_Item')->where('Timesheet_ID',$time)->where(['Entity_Identifier'=> $ProjectID, 'Entity_Class_ID' => 3])->get();
$AbsenceItems = db::table('Timesheet_Item')->where('Timesheet_ID',$time)->WhereIn('Timesheet_Item_Category_ID',[10,2,1,8,41,37,36,96,19])->get();
$Timesheet = db::table('Timesheet')->where('Timesheet_ID',$time)->first();
$TimesheetPeriod = db::table('Timesheet_Period')->where('Timesheet_Period_ID',$Timesheet->Timesheet_Period_ID)->first();
$Role = DB::table('LPT2_Links')->where('Contact',$C_ID)->join('LPT2_WBS','LPT2_Group','ID')->first();
$arrMon = [0];
$arrTue = [0];
$arrWed = [0];
$arrThu = [0];
...
<table>
	...
	<tr>
		...
		<td colspan="5" width="45">{{$Name}}</td>
		<td colspan="1" width="15">Company:</td>
		<td colspan="3" width="30">{{base64_decode($Company)}}</td>
		<td colspan="1" width="10">Role:</td>
		<td colspan="4" width="40">{{$Role->Role ?? ''}}</td>
	</tr>
	<thead>
	<tr>
