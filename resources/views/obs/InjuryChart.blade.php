<div class="card-title p-2 mt-2 ml-0 mr-0 {{$OccuranceName->Class}}">Details of Injured Person: </div>
	<div class="card-body">
		...
		<div class="col">{{$Occurance->InjuredFirstName}} {{$Occurance->InjuredSurname}}</div>
		</div>
			<div class="row">
		<div class="col-3 text-right">DOB: </div>
		<div class="col">{{$Occurance->InjuredDOB}} </div>
		</div>
			<div class="row">
		<div class="col-3 text-right">Is a member of the public: </div>
		<div class="col">@if($Occurance->InjuredMember_Of_Public === true) Yes @else No @endif </div>
		</div>
			<div class="row">
		<div class="col-3 text-right">Address: </div>
		<div class="col">{{$Occurance->InjuredAddress}} </div>
		</div>
			
			<div class="row">
		<div class="col-3 text-right">Post Code: </div>
		<div class="col">{{$Occurance->InjuredPostCode}} </div>
		</div>
			
			<div class="row">
		<div class="col-3 text-right">Phone: </div>
		<div class="col">{{$Occurance->InjuredPhone}} </div>
		</div>
			<div class="row">
		<div class="col-3 text-right">Occupation: </div>
		<div class="col">{{$Occurance->InjuredOccupation}} </div>
		</div>
			<div class="row">
		<div class="col-3 text-right">Employer: </div>
		<div class="col">{{$Occurance->InjuredEmployer}} </div>
		</div>
...
<div class="card-title p-2 mt-2 ml-0 mr-0 {{$OccuranceName->Class}}">Details of Injured Person: </div>
	<div class="card-body">
		...
		<div class="">
			...
			@if(DB::table('UKHT_Occurance_Injury_Location')->where('Occurance_ID',request('id'))->exists())
			@php $Injury = DB::table('UKHT_Occurance_Injury_Location')->where('Occurance_ID',request('id'))->first(); @endphp
			<div id="svg">
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="auto" height="500px" viewBox="0 0 800 1360" xml:space="preserve">
			...
			<div class="row">
		<div class="col-3 text-right">Was the injured person advised to see their doctor or visit a hospital? : </div>
		<div class="col">@if($Occurance->InjuredHospital == true) Yes @else No @endif </div>
		</div>
...
$('[data-toggle="popover"]').popover({
	placement: 'auto'
			...
			@if($value != null)	
				...
				$('#{{$key}}').attr('fill','red');
				$('#{{$key}}').attr('fill-opacity','0.4');
				
				$('#{{$key}}').attr('data-toggle','popover')
				$('#{{$key}}').attr('data-title',string)
