	...
	$(document).ready(function(){
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
          ...
          initialView: 'dayGridMonth',
			navLinks: true,
			  navLinkDayClick: function(data) {
				  
				  $('#EditDayLabel').html("<strong>Edit man hours for:</strong> " + moment(data).format('dddd Do MMMM YYYY'))
				  
				  $.post('GetManHoursDay',{ID:{{request('code')}},Day:moment(data).format('YYYY-MM-DD')}).done(function(result){
			
			
						$('#Input_Agency_Female').val(result.Agency_Female);
						$('#Input_Agency_Female_No').val(result.Agency_Female_No);
						$('#Input_Agency_Male').val(result.Agency_Male);
						$('#Input_Agency_Male_No').val(result.Agency_Male_No);
						$('#Input_Delivery').val(result.Delivery);
						$('#Input_Delivery_No').val(result.Delivery_No);
						$('#Input_HTUK_Female').val(result.HTUK_Female);
						$('#Input_HTUK_Female_No').val(result.HTUK_Female_No);
						$('#Input_HTUK_Male').val(result.HTUK_Male);
						$('#Input_HTUK_Male_No').val(result.HTUK_Male_No);
						$('#Input_Sub_Female').val(result.Sub_Female);
						$('#Input_Sub_Female_No').val(result.Sub_Female_No);
						$('#Input_Sub_Male').val(result.Sub_Male);
						$('#Input_Sub_Male_No').val(result.Sub_Male_No);
						$('#Input_Date_Field').val(moment(data).format('YYYY-MM-DD'));
						$('#Input_Visitors').val(result.Visitors);
						$('#Input_Visitors_No').val(result.Visitors_No);
    ...
    console.log(data);
      ...
      if (bool){
		  ...
		  $('.fc-daygrid-day-number').each(function(){
			 ...
			 $(this).prepend('<small><i class="fal fa-pencil-alt ml-auto"></i></small> ')
...
$('#CardTitle').text("Statistics for: " + moment(date).format('MMMM YYYY'))
	...
	$.post('ManHoursGetCardData',{ID:{{request('code')}}, Start:Start, End:End}).done(function(result){
