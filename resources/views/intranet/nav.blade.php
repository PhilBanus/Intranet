        ...
        <div class=" mx-auto bg-transparent border-0 p-2 m-0 d-flex"><img src="{{session('MY_PHOTO')}}"
		 ...
		 <div class="card-body p-0 m-0">
          ...
          <ul class="collapsible collapsible-accordion">
			  ...
			  @else
  <li><a class="collapsible-header waves-effect arrow-r" href="{{url('/home')}}"><i class="fas fa-home"></i> Intranet Dashboard </a>
...
@endif
			  ...
			  <li><a class="collapsible-header waves-effect arrow-r"><i class="fas fa-globe"></i> Useful Contacts <i class="fas fa-angle-down rotate-icon"></i></a>
              <div class="collapsible-body">
                <ul>
                <?php 
					$Links = DB::table("UKHT_Useful_Contacts")->get();
			  ...
			  <li><a class="collapsible-header waves-effect arrow-r" href="{{url('/CorporateCompliance')}}"><i class="fas fa-clipboard-check"></i> Corporate Compliance </a>
              ...
              <div class="collapsible-body">
                ...
                <?php 
					...
					foreach($Navs as $Nav){
						...
						echo "<li><a href='".url('/'.$Nav->Link)."' class='waves-effect'><span class='sv-slim'> {$Nav->ShortName} </span>
    ...
    <nav class="navbar fixed-top navbar-toggleable-md navbar-expand-lg scrolling-navbar double-nav">
      ...
      <!-- Breadcrumb-->
		  ...
		  <a class="navbar-brand pl-4 d-none d-md-block" href="home">
     <img src="{{ asset('./images/intranet.png') }}" height="40" class="d-inline-block align-top">
...
function imsSearch(str) {
  ...
  xmlhttp.send();
	...
	$('#IMSSearcher').on('keyup keypress keydown',function(e) {
...
<?php 
	...
	$Projects = DB::table("UKHT_Alerts")->join("UKHT_Alert_Recipients","UKHT_Alert_Recipients.Alert_ID","=","UKHT_Alerts.ID")->where("UKHT_Alert_Recipients.Contact_ID","=",session('MY_ID'))->where("Active","=","1")->where('Read',0)->orderby('Read','asc')->orderby('Alert_ID','desc');
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable" role="document">
    <div class="modal-content">
      ...
      <div class="row mt-2 mb-3 d-flex justify-content-center">
		  ...
		  <button data-dismiss="modal" class="btn btn-lg primary-color-dark text-white" onClick="markAsReadAlert({{$Project->Alert_ID}},{{session('MY_ID')}},$('#andiv{{$Project->ID}}'))" >
              @if($Project->Type === "fas fa-exclamation-triangle")
...
function markAsReadAlert(id,user,item){
		...
		$.post("readAlert",{"_token": "{{ csrf_token() }}",ID: id, USER: user}).done(function(result){
						console.log(result);
						console.log(item);
