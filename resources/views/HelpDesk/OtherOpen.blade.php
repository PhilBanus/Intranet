...
@include('HelpDesk.AdminNav')
	...
	<?php $Technicians = DB::table('HELPDESK_Calls')->Distinct('Technician')->whereNotNull('Technician')->get();

foreach($Technicians as $Technician){
	?>
<div class="card bg-transparent mt-5 border-0 h-100">

<div class="card-header bg-transparent font-weight-bold border-primary">{{DB::table('Contact')->where('Contact_ID',$Technician->Technician)->first()->Forename}} {{DB::table('Contact')->where('Contact_ID',$Technician->Technician)->first()->Surname}}</div>

	
	<div class="card-body ">
	<?php 
		
		$Calls = db::table('HELPDESK_Calls')->where('Technician',$Technician->Technician)->whereIn('Status',[1,2,3,5])->get()->take(50);
