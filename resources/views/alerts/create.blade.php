...
<div class="card">
	...
	<div class="card-title">Images <small>- click to copy link</small> </div>
		...
		<div class="w-100 row flex-row flex-nowrap" style="overflow: auto">
			...
			foreach($images as $image){
				if($image->getFilename() === "Thumbs.db"){}else{
 ...
 $height = $data[1]
				...
				?>
			<div class="border p-2 m-2" style="height: 100px; width: auto"><img   class="h-100" onClick="copyImageval('{{ asset('alertImages/' . $image->getFilename()) }}')" style="width: auto" data-value="{{ asset('alertImages/' . $image->getFilename()) }}" src="{{ asset('alertImages/' . $image->getFilename()) }}"></div>
	...
	<div class="card-footer p-0 m-0">
	 ...
	 <form action="{{ url('alertImageUploads') }}" method="post" enctype="multipart/form-data" class="md-form p-0 m-0 row col-md-6 ">
		 @csrf
...
<section class="form-light">
  ...
  <div class="card">
    ...
    <div class="card-body mx-4">
      ...
      <!--Body-->
      <div id="form" class="md-form needs-validation" novalidate>
        <input type="text" id="Subject" required class="form-control" autocomplete="off">
        <label for="Subject">Subject</label>
		 
		  
		  <select class="mdb-select validate  md-form" id="Color">
  <option value="" disabled selected>Please Select...</option>
  <option value="HSQE-Health text-white font-weight-bold" class="yellow">Health and Safety</option>
  <option value="HSQE-Quality text-white font-weight-bold" class="yellow">Quality</option>
  <option value="HSQE-Environment text-white font-weight-bold" class="yellow">Environment</option>
  ...
  <option value="success-color" class="yellow">green</option>
		  ...
		  <select class="mdb-select validate  md-form" id="Type">
  <option value="" disabled selected>Please Select...</option>
  ...
  <option value="fas fa-info-circle" class="red">Notification</option>
		   ...
		   <select class="mdb-select validate  md-form" id="From">
  <option value="" disabled selected>Please Select...</option>
  <option value="communications@hochtief.co.uk" class="bg-blue">communications@hochtief.co.uk</option>
  <option value="hsqe@hochtief.co.uk" class="bg-blue">hsqe@hochtief.co.uk</option>
		  ...
		  <select class="mdb-select validate  md-form" required multiple  id="NotificationGroup">
    ...
    <label class="form-check-label" for="Email">Do you want to Email Alert/Notification?</label>
      ...
      <div class="row d-flex align-items-center mb-4">
        ...
        <div class="col-md-6 col-md-12 text-center">
          <button type="submit" class="btn btn-pink btn-block btn-rounded z-depth-1" id="Submit">Send Alert/Notification</button>
        </div>
				...
				if(!str || !str.length ){
					 proceed = false;
														  ...
														  if(!str.length && !str.replace(/\s/g, '').length){
															 proceed = false;
															  $(this).addClass('border-danger colorful-select dropdown-danger')
															  $(this).addClass('colorful-select dropdown-danger')
															  $(this).parent('div').addClass('border-danger colorful-select dropdown-danger'); 
  ...
  var Email = true;
    ...
    console.log('This was logged in the callback: ' + result); 
			...
			var NotificationGroup = encodeURIComponent($('#NotificationGroup').val());
					$.post("sendAlert",{Subject: Subject, Body: Body, Color: Color, Type: Type, NotificationGroup: NotificationGroup, From: From, Email: Email}).done(function(result){
						console.log(result);
